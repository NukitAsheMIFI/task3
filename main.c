...
void plane(char **arr, size_t len, int dir);
...
int main(int argc, char **argv){
	...
	while (token != NULL){
		...
		if (i + 1 > (int)arr_size){
			arr_size *= 2;
			char **temp_arr = realloc(arr, arr_size);
			if (arr == NULL){
				perror("allocation memory error");
				free(arr);
				free(text);
...
void plane(char **arr, size_t len, int dir){ //сортировка пузырьком с параметром направления
